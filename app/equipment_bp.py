...
from sqlalchemy.orm import joinedload, selectinload # Убедитесь, что оба импортированы
...
def index():
    ...
    equipment_query = equipment_query_builder.perform().options(
        joinedload(Equipment.category), # Жадная загрузка категории
        selectinload(Equipment.responsible_persons) # Жадная загрузка ответственных лиц
    )
    ...
    return render_template(
        ...
        current_filters={
            ...
            'purchase_date_from': purchase_date_from_str,
            'purchase_date_to': purchase_date_to_str,
            'sort_by': sort_by_param,
            'sort_direction': sort_direction_param
        },
...
def get_eligible_responsible_users(): # Вспомогательная функция
    eligible_roles_q = db.session.query(Role.id).filter(Role.name.in_(['Admin', 'TechSpecialist']))
    eligible_role_ids = [role_id_tuple[0] for role_id_tuple in eligible_roles_q.all()]
    if not eligible_role_ids: return []
    users_query = db.select(User).filter(User.role_id.in_(eligible_role_ids)).order_by(User.last_name, User.first_name)
    return db.session.execute(users_query).scalars().all()
...
def new():
    ...
    form.responsible_person_ids.choices = [(user.id, f"{user.full_name} ({user.login})") for user in eligible_users]
    if len(form.category_id.choices) <= 1: flash("Категории не найдены.", "warning")
    if not form.responsible_person_ids.choices: flash("Пользователи (Admin/TechSpecialist) для назначения ответственными не найдены.", "warning")
...
def create():
    ...
    if form.validate_on_submit():
        try:
            ...
            selected_responsible_ids = form.responsible_person_ids.data
            if selected_responsible_ids:
                responsible_users_to_assign = [u for u in eligible_users if u.id in selected_responsible_ids]
                new_equipment.responsible_persons.extend(responsible_users_to_assign)
    ...
    else: flash('Исправьте ошибки в форме.', 'warning')
    return render_template('equipment/new.html', form=form, title="Добавить оборудование")
...
def show(equipment_id):
    equipment = db.session.query(Equipment).options(
        ...
        selectinload(Equipment.responsible_persons),
    ).filter(Equipment.id == equipment_id).first_or_404()
    
    service_records_query = equipment.service_history.options(
        joinedload(ServiceHistory.performed_by)
    ).order_by(ServiceHistory.service_date.desc())
    service_records = service_records_query.all()

    service_form = None
    if current_user.is_authenticated and current_user.role and \
       (current_user.role.name == 'Admin' or current_user.role.name == 'TechSpecialist'):
...
def edit(equipment_id):
    ...
    form.responsible_person_ids.data = [rp.id for rp in equipment.responsible_persons]
        ...
        try:
            ...
            if image_file and image_file.filename:
                ...
                if old_image_obj and old_image_id != new_image_id:
                    other_equipment_with_old_image = db.session.execute(db.select(Equipment).filter(Equipment.image_id == old_image_id, Equipment.id != equipment_id)).first()
                    if not other_equipment_with_old_image:
                        ...
                        if os.path.exists(old_image_path):
                            ...
                            except OSError as e_rm: current_app.logger.error(f"Err removing: {e_rm}")
            ...
            if equipment.inventory_number != form.inventory_number.data:
                conflicting = db.session.execute(db.select(Equipment).filter_by(inventory_number=form.inventory_number.data).filter(Equipment.id != equipment_id)).scalar_one_or_none()
                if conflicting: form.inventory_number.errors.append("Инв. номер занят."); raise ValueError("Инв. номер занят.")
            ...
            selected_ids = form.responsible_person_ids.data
            equipment.responsible_persons.clear()
            if selected_ids: equipment.responsible_persons.extend([u for u in eligible_users if u.id in selected_ids])
